      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <button
              onClick={() => {
                // Generate CSV from suppliers data
                if (!suppliers || suppliers.length === 0) {
                  alert('No suppliers to export');
                  return;
                }

                const headers = ['Name', 'Category', 'Contact Person', 'Phone', 'Email', 'Website', 'City', 'Address', 'Products', 'Rating', 'Verified'];
                const csvRows = [
                  headers.join(','),
                  ...suppliers.map(s => [
                    `"${(s.name || '').replace(/"/g, '""')}"`,
            ...
            >
              <div className="p-6 card-surface rounded-2xl border border-transparent hover:-translate-y-1 transition-all duration-300 text-center cursor-pointer">
                <div className="w-12 h-12 bg-gray-900 dark:bg-primary rounded-xl mx-auto mb-3 flex items-center justify-center text-2xl text-white shadow-soft">
                  📊
                </div>
                <span className="font-semibold text-gray-900 dark:text-foreground">
